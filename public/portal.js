...
function renderResult(application) {
    const modeLabel = application.launch_mode === 'sso'
        ? 'SSO'
        : 'Login Required';

    return `
        <a href="${application.launch_url}" class="search-result flex items-start gap-3 rounded-2xl border border-transparent px-4 py-3 hover:border-brand-400/20 hover:bg-white/5">
            ...
            <div class="min-w-0 flex-1">
                <div class="flex items-center justify-between gap-3">
                    <h4 class="truncate text-sm font-bold text-white">${application.name}</h4>
                    <span class="shrink-0 rounded-full bg-brand-400/10 px-2.5 py-1 text-[11px] font-semibold text-brand-200">${application.badge ?? ''}</span>
                </div>
                <p class="mt-1 text-sm text-slate-400">${application.description ?? ''}</p>
                <p class="mt-2 text-xs text-slate-500">${application.keywords.join(' • ')}</p>
                <p class="mt-1 text-xs font-medium text-slate-500">Mode: ${modeLabel}</p>
            </div>
        </a>
    `;
}
    ...
    if (results.length === 0) {
        searchResults.innerHTML = `
            <div class="rounded-2xl px-4 py-5 text-center">
                <p class="text-sm font-semibold text-white">Aplikasi tidak ditemukan</p>
